...
#define DRAM_SIZE (MEMORY * 1024 * 1024)
...
static const struct device_region devices[] = {
  {VC_MULT__ADDRESS, VC_MULT__SIZE, mult_load, mult_store}, /* 0x7e000000 */// MS
  ...
  {VC_SDCO__ADDRESS, VC_SDCO__SIZE, sdco_load, sdco_store}, /* 0x7ee00000 */// SD
...
vc4_emul_load (void *user_data, uint32_t address, int size) {
  ...
  if (is_in_region (address, size, BOOTROM_BASE_ADDRESS, BOOTROM_ADDRESS_END)) {
    ...
    //if (is_in_region (address, size, BOOTROM_BASE_ADDRESS + 0x7000, BOOTROM_ADDRESS_END)) {
    //  print_log ("BootRom Read(R, 4) %08x => %08x\n", address, value);
    //}
  } else if (is_in_region (address & SIZE_1GB, size, 0x0, DRAM_SIZE)) {
...
vc4_emul_store (void *user_data, uint32_t address, int size, uint32_t value) {
  ...
  if (is_in_region (address, size, BOOTROM_BASE_ADDRESS, BOOTROM_ADDRESS_END)) {
    ...
    //if (is_in_region (address, size, BOOTROM_BASE_ADDRESS + 0x7000, BOOTROM_ADDRESS_END)) {
    //  print_log ("BootRom Write(W, 4) %08x <= %08x\n", address, value);
    //}
  } else if (is_in_region (address & SIZE_1GB, size, 0x0, DRAM_SIZE)) {
