 ...
 */
uint32_t vc4_emul_load (void *user_data, uint32_t address, int size);
...
void vc4_emul_store (void *user_data,
                     uint32_t address, int size, uint32_t value);
...
__attribute__ ((noreturn))
     ...
     uint32_t
     vc4_emul_get_scalar_reg (struct vc4_emul *emul, int reg);
