...
static void print_stackline(unsigned int addr_start, int line) {
  ...
  mvwprintw(mywin, line+2, 14, "%02X %02X %02X %02X", data[0x00], data[0x01], data[0x02], data[0x03]);
  wattroff(mywin, COLOR_PAIR(3)|A_BOLD);
  mvwprintw(mywin, line+2, 27, "%c%c%c%c", GET_BYTE(data[0x00]), GET_BYTE(data[0x01]), GET_BYTE(data[0x02]), GET_BYTE(data[0x03]));
}
